...
(defn walk [i sp ep]
  ...
  (let [r (create-route sp)
    ...
    (println "start color " start-color)
    (println "  end color " (get-point-color image ep))
    (cond 
      (on-path ep start-color image)  
         (walk-route routes image sp ep start-color already-visited)
      :else {:error "start and end points are not the same color" :distance 0}
    )
))
